...
class TestThatDefiningRaceShould {
    ...
    void keepAlreadyDefinedProfessionWhenRaceIsNotForbidden() {
        ...
        final ForLoadingSnapshot forLoadingSnapshot = _ -> Result.success(Option.some(baseSnapshot));
    ...
    void successfullyChangeFromOneRaceToAnother() {
        final ForLoadingSnapshot forLoadingSnapshot = _ -> Result.success(Option.some(SnapshotFixture.getWithRace(RaceName.DWARF)));
    ...
    void failWhenNoCharacterNameIsSpecified() {
        ...
        new DefineRace(forLoadingSnapshot, this.forStoringSnapshot, this.forLoadingRace)
                ...
                .onSuccess(name -> fail("Defining null race should not succeed. Name: "+ name))
                .onFailure(cause -> assertThat(cause).isEqualTo(Causes.composite(Result.failure(new SupraCause.NotFound("character", null)))));

        new DefineRace(forLoadingSnapshot, this.forStoringSnapshot, this.forLoadingRace)
                .named(RaceName.ELF)
                .toCharacterNamed("")
                .onSuccess(name -> fail("Defining null race should not succeed. Name: "+ name))
                .onFailure(cause -> assertThat(cause).isEqualTo(Causes.composite(Result.failure(new SupraCause.NotFound("character", "")))));
    }

    @Test
    @DisplayName("Fail when no character's name is specified")
    void failWhenCharacterNameDoesNotExist() {
        final ForLoadingSnapshot forLoadingSnapshot = _ -> Result.success(Option.none());

        new DefineRace(forLoadingSnapshot, this.forStoringSnapshot, this.forLoadingRace)
                .named(RaceName.ELF)
                .toCharacterNamed(DefaultCharacterData.NAME)
                .onSuccess(name -> fail("Defining null race should not succeed. Name: "+ name))
                .onFailure(cause -> assertThat(cause).isEqualTo(Causes.composite(Result.failure(new SupraCause.NotFound("character", DefaultCharacterData.NAME)))));
    }
}
